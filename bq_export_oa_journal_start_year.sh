...
bq query \
    ...
    --max_rows=0 \
    'select journal_issn_l as issn_l, year, oa_status, count(1) as num_dois from unpaywall.api_live where published_date < date_sub(current_date(), interval 1 month) group by 1, 2, 3;'
