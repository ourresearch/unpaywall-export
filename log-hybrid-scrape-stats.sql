...
create temp table tmp_hybrid_scrape_2hr as (select scrape_evidence, scrape_license, scrape_pdf_url is not null as has_pdf, scrape_metadata_url is not null as has_metadata_url from pub where scrape_updated > now() - interval '2 hours');
create temp table tmp_hybrid_scrape_8hr as (select scrape_evidence, scrape_license, scrape_pdf_url is not null as has_pdf, scrape_metadata_url is not null as has_metadata_url from pub where scrape_updated > now() - interval '8 hours');
