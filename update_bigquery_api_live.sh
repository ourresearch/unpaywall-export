...
bq load --source_format=CSV -F '\t' --schema 'data:string' $STAGING_TABLE $FILENAME
...
bq query --max_rows=0 --destination_table="$EXTRACTED_STAGING_TABLE" "$(cat bigquery_raw_transform.sql | sed "s/__API_RAW_STAGING_TABLE__/${STAGING_TABLE}/")"
require_success $? 4 'bq'

logger "delete raw staging table $STAGING_TABLE"
bq rm -f $STAGING_TABLE
require_success $? 5 'bq'

LIVE_TABLE="unpaywall.api_live"
logger "delete updated rows from live table $LIVE_TABLE"
bq query --use_legacy_sql=false "delete from $LIVE_TABLE where doi in (select doi from $EXTRACTED_STAGING_TABLE);"
...
bq rm -f $EXTRACTED_STAGING_TABLE
